...
-- contains 
-- 1. beam_step: runs a single step of beam search
-- 2. beam_search: iteratively calls beam_step to sample captions from images 
  ...
  for prev_choice = 1, divm-1 do
    ...
    for sub_beam = 1,bdash do
      for prev_labels = 1,bdash do 
        logprobsf[sub_beam][prev_decisions[prev_labels]] = logprobsf[sub_beam][prev_decisions[prev_labels]] - opt.lambda
      end
    end
  end
